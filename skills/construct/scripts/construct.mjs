...
// src/cli.ts
import { resolve as resolve8, join as join35 } from "path";
...
// src/research/metrics.ts
import { AsyncLocalStorage } from "async_hooks";
var store = new AsyncLocalStorage();
var total = { requests: 0, cacheHits: 0, bytes: 0 };
function zero() {
  return { requests: 0, cacheHits: 0, bytes: 0 };
}
function recordFetch(bytes, cacheHit = false) {
...
async function timeAngle(angle, sink, fn) {
  const local = zero();
...
async function searchViaFirecrawl(query2, limit, opts = {}) {
  const base = firecrawlBase(opts);
  ...
  const r = await postJson(base, "/search", { query: query2, limit, sources: ["web"] }, SEARCH_TIMEOUT_MS);
  if (!r.ok) {
...
function combine(names) {
  ...
  return {
    ...
    postUp: (file, run2) => found.flatMap((x) => x.postUp?.(file, run2) ?? [])
  };
}
...
function stackControl(service, action, deps = {}) {
  const run2 = deps.run ?? defaultRun;
  ...
  if (action === "down") {
    const r = run2("docker", ["compose", "-f", file, ...profiles, "down"], { timeoutMs: DOWN_TIMEOUT_MS, capture: true });
    return { message: r.ok ? `${tag}: stopped.` : `${tag}: down failed.
${r.stderr}`, code: r.ok ? 0 : 1 };
  }
  if (action === "status") {
    const r = run2("docker", ["compose", "-f", file, ...profiles, "ps"], { timeoutMs: PS_TIMEOUT_MS, capture: true });
    return { message: r.ok ? r.stdout.trim() || `${tag}: no services running.` : `${tag}: status failed.
${r.stderr}`, code: 0 };
  }
  const pulled = run2("docker", ["compose", "-f", file, ...profiles, "pull"], { timeoutMs: pullTimeoutMs() });
  if (!pulled.ok) {
    return {
...
${pulled.stderr}` : ""),
  ...
  const up = run2("docker", ["compose", "-f", file, ...profiles, "up", "-d", "--wait"], { timeoutMs: UP_TIMEOUT_MS });
  if (!up.ok) return { message: `${tag}: up failed.${up.stderr ? `
${up.stderr}` : ""}`, code: 1 };
  return { message: [`${tag}: ${spec.summary}`, ...spec.postUp?.(file, run2) ?? []].join("\n"), code: 0 };
}
...
function cacheStats(now = Date.now()) {
  const dir = cacheDir();
  const out2 = { dir, entries: 0, bytes: 0, fresh: 0, stale: 0, ttlMs: ttlMs() };
  if (!existsSync4(dir)) return out2;
  let oldest = Number.POSITIVE_INFINITY;
  let newest = 0;
  for (const name2 of readdirSync2(dir)) {
    const abs = join4(dir, name2);
    try {
...
function oneWriterFooter(runAbs, opts = {}) {
  const forbidden = opts.writingCommands?.length ? ` Do not run any engine command that writes (${opts.writingCommands.map((c2) => `\`${c2}\``).join(", ")}).` : "";
  return `
...
function listPhases(runDir, engineAbs, defs) {
  ...
  return defs.map((def) => {
    const worklist = join7(run2, def.worklist);
    const parsed = readJsonSafe(worklist);
    const ids = def.ids(parsed, run2, engineAbs);
    ...
    return {
      ...
      items: ids?.length ?? 0,
      ids: ids ?? [],
      prerequisite: def.prerequisite(run2, engineAbs, parsed),
      ...ready ? { parsed } : {}
    };
  });
}
function orchestrateRun(runDir, engineAbs, defs, contracts, opts = {}) {
  ...
  const phases = listPhases(run2, engineAbs, defs);
  ...
  const orchDir = join7(run2, "orchestration");
  const agentsDir = join7(orchDir, "agents");
  ensureDir(join7(orchDir, "out"));
  ensureDir(agentsDir);
  const written = [];
  const notices = [];
  for (const [name2, content] of Object.entries(contracts(run2, engineAbs, phases))) {
    written.push(writeArtifact(join7(agentsDir, `${name2}.md`), content));
  }
  if (!opts.eco) {
    for (const ph of selected) {
      ...
      written.push(writeArtifact(join7(orchDir, `${ph.name}.workflow.mjs`), emitWorkflowScript(ph, def, run2, engineAbs, small, opts.constants)));
    }
  }
  written.push(writeArtifact(join7(orchDir, "RUNBOOK.md"), runbookMd(phases, defs, run2, engineAbs, brand().cli, opts.runbookPreamble)));
...
function describe(root, rel2, fallbackTitle) {
  const decl = {
    uri: `${URI_SCHEME}${rel2.split(sep).join("/")}`,
    name: rel2.split(sep).join("/"),
    title: fallbackTitle,
    mimeType: "text/markdown"
  };
  const summary = firstProse(join8(root, rel2));
...
function createServer(adapter, opts = {}) {
  ...
  async function handleToolCall(msg, reply) {
    ...
    const invalid = validateArgs(decl.inputSchema, args2);
    if (invalid) {
...
configure({
  ...
  cacheTtlMs: CACHE_TTL_HOURS * 36e5,
...
// src/brief.ts
import { existsSync as existsSync7, readFileSync as readFileSync6, writeFileSync as writeFileSync5, mkdirSync as mkdirSync5 } from "fs";
...
function saveBrief(runDir, brief) {
  ...
  writeFileSync5(path, JSON.stringify(brief, null, 2));
  return path;
}
...
// src/brainstorm.ts
import { existsSync as existsSync8, readFileSync as readFileSync7, writeFileSync as writeFileSync6, mkdirSync as mkdirSync6 } from "fs";
...
function saveBrainstorm(runDir, b) {
  ...
  writeFileSync6(path, JSON.stringify(b, null, 2));
  return path;
}
function writeBrainstormMd(runDir, b) {
  ...
  writeFileSync6(path, md.endsWith("\n") ? md : md + "\n");
  return path;
}
...
function loadBrainstorm(runDir, warn = () => {
  ...
  let data;
  try {
    data = JSON.parse(readFileSync7(path, "utf8"));
  } catch (e) {
...
function excerptsFromText(text, url, title, source, question, perSource) {
  ...
  return excerptWindows(text, question, { perDoc }).map((w, i2) => ({
    source,
    ...
    title: i2 === 0 ? title : `${title} (lines ${w.start + 1}\u2013${w.end})`,
    ref: url,
...
async function discover(query2, engine, n) {
  const notes = [];
  if (engine === "firecrawl") {
    const attempt = firecrawlDown ? void 0 : await searchViaFirecrawl(query2, n);
...
async function webFetchUrls(urls, question, perSource, source = "market", fetchAll = false, concurrency = FETCH_CONCURRENCY) {
  ...
  const fetched = await pool(toFetch, concurrency, async (url) => {
    try {
      return { url, metaDescription: void 0, ...await cachedFetchAndExtract(url, { stripConsent: true }, true) };
    } catch (e) {
      return { url, text: "", note: `Could not fetch ${url}: ${e.message}`, metaDescription: void 0 };
    }
  });
...
import { readdirSync as readdirSync4, statSync as statSync4, lstatSync, readFileSync as readFileSync8, realpathSync as realpathSync2 } from "fs";
import { join as join11, sep as sep2, extname } from "path";
import { createHash } from "crypto";
import { readFileSync as readFileSync22, existsSync as existsSync9 } from "fs";
...
import { readFileSync as readFileSync62, writeFileSync as writeFileSync22 } from "fs";
import { join as join12 } from "path";
import { mkdirSync as mkdirSync22, readdirSync as readdirSync22, readFileSync as readFileSync72, rmSync as rmSync22, statSync as statSync32, writeFileSync as writeFileSync32 } from "fs";
import { dirname as dirname4, join as join13 } from "path";
import { existsSync as existsSync52, readdirSync as readdirSync32, statSync as statSync42 } from "fs";
import { join as join14 } from "path";
import { createHash as createHash3 } from "crypto";
import { existsSync as existsSync62, readFileSync as readFileSync82 } from "fs";
...
import { existsSync as existsSync32, readFileSync as readFileSync42 } from "fs";
import { join as join52 } from "path";
import { createHash as createHash2 } from "crypto";
import { existsSync as existsSync42, mkdirSync as mkdirSync7, mkdtempSync as mkdtempSync2, readFileSync as readFileSync52, renameSync as renameSync2, rmSync as rmSync4, writeFileSync as writeFileSync7 } from "fs";
import { dirname as dirname32, join as join62, resolve as resolve4, sep as sep22 } from "path";
import { gunzipSync } from "zlib";
import { join as join15 } from "path";
import { existsSync as existsSync82, mkdirSync as mkdirSync32, readFileSync as readFileSync10, writeFileSync as writeFileSync42 } from "fs";
...
function walk(root, opts = {}) {
  ...
  walking: while (stack.length) {
    ...
    let entries;
    try {
      entries = readdirSync4(frame.dir, { withFileTypes: true }).sort(
...
function resolveGrammarsTier(opts = {}) {
  ...
  const withDirs = (tier, dir) => ({
    ...
    dir,
    cacheDir: cacheDir2,
    dirs: [dir, ...existsSync9(join22(dir, "..", EXTENDED_DIR)) ? [join22(dir, "..", EXTENDED_DIR)] : []]
  });
  const legacy = process.env.CODEINDEX_GRAMMAR_DIR ?? process.env.ULTRAINDEX_GRAMMAR_DIR;
  if (legacy && legacy.trim() && existsSync9(legacy)) return withDirs("env", legacy);
  const here = opts.moduleDir ?? dirname3(fileURLToPath2(import.meta.url));
  const adjacent = [
  ...
  for (const c2 of adjacent) if (existsSync9(c2)) return withDirs("adjacent", c2);
  const env2 = process.env.CODEINDEX_GRAMMARS_DIR;
  if (env2 && env2.trim() && existsSync9(env2)) return withDirs("env", env2);
  if (existsSync9(cacheDir2)) return withDirs("cache", cacheDir2);
  return { tier: "none", cacheDir: cacheDir2, dirs: [] };
}
...
function collectAll(root, spec, defNames, maxCalls, wantImports) {
  ...
  const visit = (node) => {
    ...
    if (!literals.full) {
      const line2 = node.startPosition.row + 1;
      if (STRING_NODE.test(type) && isPlainString(node)) literals.addString(node.text, line2);
      else if (kids.length === 0 && NUMBER_NODE.test(type)) literals.add("number", node.text.trim(), line2);
      else if (REGEX_NODE.test(type)) literals.add("regex", node.text, line2);
    }
...
function readTsConfig(root, fileSet, rel2, warnings, seen) {
  ...
  seen.add(rel2);
  const cfg = tolerantJsonParse(readText(join72(root, rel2)));
  if (cfg === void 0) {
...
function buildResolveContext(scan2) {
  ...
  for (const rel2 of fileSet) {
    ...
    const text = readText(join72(scan2.root, rel2));
  ...
  for (const rel2 of fileSet) {
    ...
    const pkg = tolerantJsonParse(readText(join72(scan2.root, rel2)));
    if (pkg === void 0) {
  ...
  for (const rel2 of fileSet) {
    ...
    const composer = tolerantJsonParse(readText(join72(scan2.root, rel2)));
    if (!composer) {
...
function resolveRelations(scan2, importPairs) {
  ...
  for (const f of scan2.files) {
    ...
    for (const r of f.relations) {
      ...
      const pool2 = imported.length ? imported : cands;
      const chosen = pickCandidate(f.rel, pool2.map((d) => ({ file: d.file, lang: d.lang })));
      if (!chosen) continue;
      const target = pool2.find((d) => d.file === chosen.file);
      out2.push({
...
function buildSymbolGraph(scan2, importPairs) {
  ...
  for (const f of scan2.files) {
    ...
    for (const c2 of f.calls) {
      ...
      if (!cands.length) continue;
      const imported = cands.filter((d) => importPairs.has(`${f.rel}|${d.file}`));
      const pool2 = imported.length ? imported : family === "js" ? [] : cands;
...
function searchIndex(scan2, query2, opts = {}) {
  const terms = [];
  const seen = /* @__PURE__ */ new Set();
  for (const kw of keywords2(query2)) {
  ...
  for (const f of FIELDS) {
    let total2 = 0;
    for (const d of docs) total2 += d.fields[f].len;
    avgLen[f] = total2 / n || 1;
  }
  ...
  for (const d of docs) {
    ...
    const scored = d.decls.map((decl) => {
      const toks = new Set(subtokens2(decl.name));
      let hits2 = 0;
...
function symbolComplexity(scan2, rel2, top = 50) {
  ...
  for (const f of scan2.files) {
    ...
    const lines = readText(join82(scan2.root, f.rel)).split("\n");
    for (const s of f.symbols) {
...
function fileComplexityFor(scan2) {
  ...
  if (!c2.fileComplexity) {
    ...
    for (const f of scan2.files) {
      if (f.kind !== "code") continue;
      m.set(f.rel, complexityOfSource(readText(join92(scan2.root, f.rel))));
    }
    c2.fileComplexity = m;
  }
...
function buildGraph(scan2, ctx, modules, moduleOf, meta) {
  ...
  if (unique.size) {
    for (const f of scan2.files) {
      ...
      const content = scan2.docText.get(f.rel) ?? readText(join102(scan2.root, f.rel));
      if (!content) continue;
...
function findSymbol(scan2, namePath, opts = {}) {
  ...
  if (opts.includeBody) {
    for (const m of capped) {
      ...
      const content = readText(join112(scan2.root, m.file));
      if (!content) continue;
...
function replaceSymbolBody(scan2, namePath, body2, file) {
  ...
  writeFileSync22(abs, lines.join("\n"));
...
function writeMemory(repo, name2, content) {
  ...
  mkdirSync22(dirname4(path), { recursive: true });
...
function probeNodePkg(root, dir, kind, warnings) {
  ...
  if (!existsSync52(path)) return void 0;
...
function probeGradle(root, dir) {
  for (const f of ["build.gradle", "build.gradle.kts"]) {
    if (existsSync52(join14(root, dir, f))) {
...
function isDirAt(root, rel2) {
  try {
    return statSync42(join14(root, rel2)).isDirectory();
  } catch {
    return false;
  }
}
function subdirsOf(root, base) {
  let entries;
  try {
    entries = readdirSync32(base ? join14(root, base) : root, { withFileTypes: true });
  } catch {
    return [];
  }
...
function holderFor(symbols, line2) {
  let best;
  for (const s of symbols) {
...
function resolveEmbedModelDir(repo) {
  ...
  for (const c2 of candidates) {
    if (existsSync62(join16(c2, "model.json"))) return c2;
  }
  return void 0;
}
...
function loadEmbedModel(dir) {
  ...
  if (!existsSync62(path)) return void 0;
...
function quantize(vec) {
  ...
  const norm22 = Math.sqrt(sumsq);
  if (norm22 === 0) return out2;
  for (let d = 0; d < dim; d++) {
    let q = roundHalfToEven(vec[d] / norm22 * QUANT);
    if (q > QUANT) q = QUANT;
...
function searchSemantic(scan2, query2, index, opts = {}) {
  const limit = opts.limit ?? DEFAULT_LIMIT2;
  const lexical = searchIndex(scan2, query2, { limit: Math.max(limit, 50), fuzzy: opts.fuzzy });
  const q = opts.queryVec ?? (opts.model ? encode(opts.model, query2) : void 0);
...
async function encodeQueryViaEndpoint(query2, opts = {}) {
  const [vec] = await embedViaEndpoint([query2], opts);
...
function negotiateProtocol2(requested) {
  return typeof requested === "string" && PROTOCOL_VERSIONS2.includes(requested) ? requested : LATEST_PROTOCOL2;
}
  ...
  return JSON.stringify(
    {
      ...
      ...artifact && existsSync72(artifact) ? { artifact, artifactNote: "The full result is already on disk here \u2014 read it directly if you need all of it." } : artifact ? { artifactNote: `Run \`codeindex index --repo ${repo} --out ${join17(repo, INDEX_DIR)}\` to get this as a file.` } : {}
    },
    null,
    2
  ) + "\n";
...
var PROTOCOL_VERSIONS2;
var LATEST_PROTOCOL2;
var ANNOTATIONS_SINCE2;
var RICH_TOOLS_SINCE2;
var DEFAULT_MAX_RESPONSE_BYTES2;
var NARROWER;
var ARTIFACT_FOR;
var init_protocol = __esm({
  "src/mcp/protocol.ts"() {
    "use strict";
    init_preload();
    PROTOCOL_VERSIONS2 = ["2024-11-05", "2025-03-26", "2025-06-18", "2025-11-25"];
    LATEST_PROTOCOL2 = PROTOCOL_VERSIONS2[PROTOCOL_VERSIONS2.length - 1];
    ANNOTATIONS_SINCE2 = "2025-03-26";
    RICH_TOOLS_SINCE2 = "2025-06-18";
    DEFAULT_MAX_RESPONSE_BYTES2 = 1e6;
    NARROWER = {
...
function toolsFor(defaultRepo, protocolVersion = PROTOCOL_VERSIONS2[0]) {
...
async function callTool(name2, args2, defaultRepo) {
  ...
  if (name2 === "search") {
    const query2 = str(args2.query);
    if (!query2) throw new Error("`query` is required");
    const scan2 = getScan(repo, scanOpts, walked);
    ...
    if (args2.semantic === true) {
      ...
      if (endpoint) {
        try {
          ...
          const queryVec = await encodeQueryViaEndpoint(query2);
          const results2 = searchSemantic(scan2, query2, index, { queryVec, limit, fuzzy });
          return JSON.stringify({ results: results2, tier: "endpoint" }, null, 2);
        } catch (e) {
          const results2 = searchIndex(scan2, query2, { limit, fuzzy, ...rankOpt });
          return JSON.stringify(
            { results: results2, tier: "lexical", degradedReason: `embedding endpoint failed: ${errMessage2(e)}` },
            null,
            2
          );
        }
      ...
      if (model) {
        ...
        const results2 = searchSemantic(scan2, query2, index, { model, limit, fuzzy });
        return JSON.stringify({ results: results2, tier: "static" }, null, 2);
      }
      const results = searchIndex(scan2, query2, { limit, fuzzy, ...rankOpt });
      return JSON.stringify(
    ...
    return JSON.stringify(searchIndex(scan2, query2, { limit, fuzzy, ...rankOpt }), null, 2);
  }
  if (name2 === "embed_status") {
    const modelDir = resolveEmbedModelDir(repo);
    const model = modelDir ? memoizedEmbedModel(modelDir) : void 0;
    const endpoint = resolveEmbedEndpoint();
    const mode2 = endpoint ? "endpoint" : model ? "static" : "none";
    const status = {
...
async function runMcpServer(opts = {}) {
  ...
  let protocolVersion = PROTOCOL_VERSIONS2[0];
  ...
  async function handle2(req) {
    ...
    try {
      if (req.method === "initialize") {
        protocolVersion = negotiateProtocol2(req.params?.protocolVersion);
        ...
        try {
          ...
          const invalid = decl ? validateArgs2(decl.inputSchema, args2) : void 0;
          if (invalid) throw new Error(invalid);
          const raw = await callTool(name2, args2, opts.defaultRepo);
          const repo = str(args2.repo) ?? opts.defaultRepo ?? "";
          const text = capResponse2(raw, name2, repo, opts.maxResponseBytes ?? DEFAULT_MAX_RESPONSE_BYTES2);
          const capped = text !== raw;
          const link = capped && protocolVersion >= RICH_TOOLS_SINCE2 ? resourceLinkFor(text, name2) : void 0;
          const structured = protocolVersion >= RICH_TOOLS_SINCE2 ? structuredContentFor2(text, capped, OUTPUT_SCHEMAS[name2] !== void 0) : void 0;
          send({
            id: req.id,
            result: {
...
function resolveEngineUrl() {
  try {
    const here = fileURLToPath22(import.meta.url);
    if (here.endsWith("engine.mjs")) return pathToFileURL(here).href;
    const adjacent = join42(dirname22(here), "engine.mjs");
...
function tagsQueryStatus(key) {
  const present = resolveGrammarsTier().dirs.some((d) => existsSync32(join52(d, `${key}.tags.scm`)));
...
function extractTags(ext, content) {
  ...
  const query2 = queryFor(key, language);
  if (!query2) return [];
  let tree = null;
  try {
    tree = parser2.parse(content);
...
function extractTarInto(rawTar, destDir) {
  ...
  for (const entry of readTar(asBuffer(rawTar))) {
    ...
    writeFileSync7(dest, entry.data);
    written.push(rel2);
  }
  return written;
}
...
`);
  ...
  const runtime = join62(cacheDir2, "web-tree-sitter.wasm");
  const markerPath = join62(dirname32(cacheDir2), `${ENGINE_VERSION}.sha256`);
  if (existsSync42(runtime) && expected && existsSync42(markerPath)) {
    let marker = "";
    try {
...
`
  ...
  let tmp;
  try {
    mkdirSync7(dirname32(cacheDir2), { recursive: true });
    tmp = mkdtempSync2(join62(dirname32(cacheDir2), ".grammars-tmp-"));
    extractGrammarsTarball(bytes, tmp);
    if (!existsSync42(join62(tmp, "web-tree-sitter.wasm"))) {
      throw new Error("archive is missing web-tree-sitter.wasm");
    }
    if (existsSync42(cacheDir2)) rmSync4(cacheDir2, { recursive: true, force: true });
    renameSync2(tmp, cacheDir2);
    tmp = void 0;
    if (expected) writeFileSync7(markerPath, expected + "\n");
  } catch (e) {
    if (tmp) {
      try {
...
function renderScip(scan2, opts = {}) {
  ...
  let total2 = 0;
  for (const d of documents) total2 += d.length;
  const index = new Bytes(total2 + metadata2.length + 16);
...
function deltaFor(repo, graph, symbols, opts = {}) {
  if (!have2("git")) return { error: "git is required for delta and was not found on PATH" };
  if (!isGitWorktree(repo)) return { error: `delta needs a git worktree \u2014 ${repo} is not inside one` };
  const notes = [];
  let base;
  if (opts.staged) {
    const head = sh2("git", ["-C", repo, "rev-parse", "HEAD"]);
    if (!head.ok) return { error: "cannot resolve HEAD \u2014 empty repository?" };
...
function parseMcpFlags(argv) {
  ...
  if (defaultRepo && !existsSync82(defaultRepo)) throw new Error(`--repo path does not exist: ${defaultRepo}`);
...
async function runCli(rawArgv) {
  ...
  if (!existsSync82(flags2.repo)) throw new Error(`--repo path does not exist: ${flags2.repo}`);
  ...
  if (cmd === "index") {
    ...
    const cachePath2 = join20(outDir, "cache.json");
    let cache;
    let meta = {};
    try {
      const parsed = JSON.parse(readFileSync10(cachePath2, "utf8"));
    ...
    if (fastpath) {
      if (scan2.cacheDirty) writeCache2(meta);
...
`
      ...
      writeCache2({ graphSha1: sha1(graphJson), symbolsSha1: sha1(symbolsJson), embed: embedMeta });
...
`);
    ...
    if (sub === "status") {
      ...
      const present = (name2) => info2.dirs.some((d) => existsSync82(join20(d, name2)));
...
async function embedOneChunk(text) {
  const v = await embedOne(text.slice(0, 4e3), { base: OLLAMA, model: EMBED_MODEL });
...
async function semanticRescore(results, query2) {
  ...
  const qv = await embedOneChunk(query2);
...
import { existsSync as existsSync10, mkdirSync as mkdirSync8, readFileSync as readFileSync11, writeFileSync as writeFileSync8 } from "fs";
...
function contentKey(it) {
  return `${it.source}|${it.location || it.url || it.ref}`;
}
function emptyLedger() {
  return { schemaVersion: ID_LEDGER_SCHEMA_VERSION, next: 1, assigned: {} };
}
function loadLedger(dir) {
  const p = join21(dir, "ids.json");
  if (!existsSync10(p)) return emptyLedger();
  try {
    ...
    return { schemaVersion: ID_LEDGER_SCHEMA_VERSION, next, assigned };
  } catch {
    return emptyLedger();
  }
}
function assignIds2(results, ledger = emptyLedger()) {
  ...
  return flat.map((it) => {
    ...
    let id = ledger.assigned[key];
    if (!id) {
      id = `E${ledger.next++}`;
      ledger.assigned[key] = id;
    }
    return { id, ...it };
  });
}
function evidenceFingerprint(evidence) {
  ...
  for (const e of [...evidence].sort((a, b) => a.id.localeCompare(b.id))) {
    h.update(`${e.id}\0${contentKey(e)}\0${e.snippet.length}
`);
  }
...
function renderEvidenceMarkdown(evidence, meta) {
  ...
  if (meta.timings?.length) {
    out2.push(`## Retrieval cost`);
    out2.push("");
    out2.push(`| Angle | Wall-clock | Requests | Cache hits | Bytes |`);
    out2.push(`|---|---:|---:|---:|---:|`);
    for (const t of meta.timings) {
      out2.push(`| ${t.angle} | ${(t.ms / 1e3).toFixed(1)} s | ${t.requests} | ${t.cacheHits} | ${fmtBytes(t.bytes)} |`);
    }
    out2.push("");
  }
  return out2.join("\n");
...
function writeDossier(dir, evidence, meta, ledger = emptyLedger()) {
  ...
  const idsJson = join21(dir, "ids.json");
  ...
  writeFileSync8(idsJson, JSON.stringify(ledger, null, 2));
  return { dir, evidenceJson, evidenceMd, metaJson, idsJson };
}
...
function capSource(r, perSource) {
  const pinned = r.items.filter((i2) => i2.meta?.pinned);
  const rest = [...r.items.filter((i2) => !i2.meta?.pinned)].sort((a, b) => b.score - a.score);
...
async function runResearch(ctx, builtAt) {
  ...
  const dir = join23(ctx.runDir, "evidence");
...
// src/render.ts
import { existsSync as existsSync12, mkdirSync as mkdirSync9, readFileSync as readFileSync13, writeFileSync as writeFileSync10, rmSync as rmSync5 } from "fs";
import { join as join26, dirname as dirname5 } from "path";

// src/srd.ts
import { join as join24 } from "path";
function srdManifestPath(runDir) {
  return join24(runDir, "SRD.json");
}
function pad3(n) {
...
function seedTokens(brief) {
  const brand2 = brief.design?.brandConstraints?.trim();
  const byCategory = {
    color: [
      { category: "color", name: "color.bg", value: "#ffffff", note: brand2 ? `Adjust to brand: ${brand2}` : "Primary surface" },
...
function buildPlanPath(runDir) {
  return join25(runDir, "BUILD-PLAN.json");
}
...
function writePlan(runDir, plan) {
  const path = buildPlanPath(runDir);
  writeFileSync9(path, JSON.stringify(plan, null, 2) + "\n");
  return path;
}

// src/render.ts
function writeFile(out2, rel2, content, files) {
  const abs = join26(out2, rel2);
  mkdirSync9(dirname5(abs), { recursive: true });
  writeFileSync10(abs, content.endsWith("\n") ? content : content + "\n");
  files.push(rel2);
}
...
function emitSRD(srd, opts) {
  const files = [];
  const out2 = opts.out;
  if (!opts.prd && !opts.noPrd && existsSync12(join26(out2, "requirements", "prd"))) {
  ...
  rmSync5(join26(out2, "architecture", "decisions"), { recursive: true, force: true });
  rmSync5(join26(out2, "design"), { recursive: true, force: true });
  rmSync5(join26(out2, "prd"), { recursive: true, force: true });
  ...
  rmSync5(join26(out2, "requirements", "prd"), { recursive: true, force: true });
  if (opts.prd) {
    for (const fr of srd.functional) {
  ...
  writeFileSync10(srdManifestPath(out2), JSON.stringify(srd, null, 2) + "\n");
  files.push("SRD.json");
  if (opts.merge) {
    writeFile(out2, "SRD.md", renderMergeBundle(srd), files);
  } else {
    rmSync5(join26(out2, "SRD.md"), { force: true });
  }
  return { dir: out2, files, srd };
}

// src/check.ts
import { existsSync as existsSync14, readFileSync as readFileSync15, readdirSync as readdirSync5, statSync as statSync6 } from "fs";
import { join as join28, relative, sep as sep3 } from "path";
...
function runReview(runDir, opts = {}) {
  const manifest = srdManifestPath(runDir);
  if (!existsSync13(manifest)) throw new Error(`No SRD.json in ${runDir} \u2014 render the SRD first (construct render).`);
  let srd;
  try {
    srd = JSON.parse(readFileSync14(manifest, "utf8"));
  } catch (e) {
    throw new Error(`SRD.json is unreadable: ${e.message}`);
  }
  const evidence = loadEvidence(join27(runDir, "evidence", "evidence.json"));
  ...
  const rank2 = normalizedRank(pairs);
  ...
  writeFileSync11(join27(runDir, "VERIFY.todo.json"), JSON.stringify(todo, null, 2));
  writeFileSync11(join27(runDir, "VERIFY.md"), renderWorklistMd(worklist, pairs.length, dropped));
  return worklist;
}
function renderWorklistMd(wl, total2, dropped) {
  ...
  if (dropped.length) {
    out2.push("");
    out2.push(`> **DROPPED (--max-review): ${dropped.length} of ${total2} pair(s) are NOT in this worklist and will NOT be adjudicated.**`);
...
function applyVerdicts(runDir, verdictsPath) {
  if (!existsSync13(verdictsPath)) throw new Error(`verdicts file not found: ${verdictsPath}`);
  let raw;
  try {
    raw = JSON.parse(readFileSync14(verdictsPath, "utf8"));
  } catch (e) {
  ...
  const todoPath = join27(runDir, "VERIFY.todo.json");
  ...
  writeFileSync11(join27(runDir, "VERIFY.json"), JSON.stringify({ ...result, ...srdGeneratedAt ? { srdGeneratedAt } : {}, verdicts }, null, 2));
...
function normalizedRank(pairs) {
  ...
  return (p) => {
    ...
    const i2 = scale.indexOf(p.score);
    return i2 < 0 ? 0 : 1 - i2 / (scale.length - 1);
  };
}
function readSrdGeneratedAt(runDir) {
  const p = join27(runDir, "SRD.json");
  if (!existsSync13(p)) return void 0;
  try {
    const srd = JSON.parse(readFileSync14(p, "utf8"));
...
function mdFiles(runDir) {
  ...
  while (stack.length) {
    ...
    for (const name2 of entries) {
      ...
      const rel2 = relative(runDir, abs).split(sep3).join("/");
      if (st.isDirectory()) {
        if (rel2 === "evidence" || rel2 === "orchestration" || name2 === ".construct") continue;
        stack.push(abs);
      } else if (name2.endsWith(".md")) {
        if (rel2 === "VERIFY.md") continue;
        out2.push(rel2);
      }
    }
  }
...
function countProposedIdeas(runDir) {
  const p = join28(runDir, "brainstorm.json");
  if (!existsSync14(p)) return 0;
  try {
    const data = JSON.parse(readFileSync15(p, "utf8"));
...
function loadEvidence2(runDir) {
  const path = join28(runDir, "evidence", "evidence.json");
...
function applySemantic(runDir, result, allowUnverified) {
  const p = join28(runDir, "VERIFY.json");
...
function checkDesign(runDir, srd, errors, warnings) {
  ...
  for (const f of DESIGN_REQUIRED_FILES) {
    if (!existsSync14(join28(runDir, f))) errors.push(`Missing required design file: ${f} (re-render at --level complex).`);
  }
  ...
  const tokenDoc = join28(runDir, "design", "DESIGN-TOKENS.md");
  if (existsSync14(tokenDoc) && readFileSync15(tokenDoc, "utf8").includes(DESIGN_TOKENS_SEEDED_BANNER)) {
...
function checkModules(runDir, srd, errors, warnings) {
  ...
  if (!existsSync14(join28(runDir, "prd", "README.md"))) {
    errors.push(`Missing required module-PRD index: prd/README.md (re-render).`);
  }
  for (const m of mods) {
    if (!existsSync14(join28(runDir, "prd", m.id, "PRD.md"))) {
...
function checkRun(runDir, opts = {}) {
  ...
  for (const f of REQUIRED_FILES) {
    if (!existsSync14(join28(runDir, f))) errors.push(`Missing required file: ${f} (run \`construct render --out ${runDir}\`).`);
  }
  const manifest = srdManifestPath(runDir);
  ...
  let srd;
  try {
    srd = JSON.parse(readFileSync15(manifest, "utf8"));
  } catch (e) {
    errors.push(`SRD.json is unreadable: ${e.message}`);
    return { ok: false, structural: { ok: false, errors, warnings }, coverage: emptyCoverage };
  }
  for (const rel2 of mdFiles(runDir)) {
    const text = readFileSync15(join28(runDir, rel2), "utf8");
  ...
  const titleSeen = /* @__PURE__ */ new Map();
  for (const fr of srd.functional) {
  ...
  if (opts.minGrounding !== void 0) {
    const total2 = coverage.frTotal + coverage.nfrTotal + coverage.adrTotal;
    const grounded = coverage.frGrounded + coverage.nfrGrounded + coverage.adrGrounded;
    const actualPct = total2 === 0 ? 0 : Math.round(grounded / total2 * 100);
  ...
  if (opts.semantic) {
    ...
    result.semanticSkipped = { citedClaims, verifyExists: existsSync14(join28(runDir, "VERIFY.json")) };
  }
  return result;
}
function pct(part, total2) {
  if (total2 === 0) return "n/a";
  return `${Math.round(part / total2 * 100)}%`;
}
...
function loadMetaNotes(runDir) {
  const path = join29(runDir, "evidence", "meta.json");
  if (!existsSync15(path)) return [];
  try {
    const meta = JSON.parse(readFileSync16(path, "utf8"));
...
function analyzeRun(runDir) {
  ...
  const shellQuote2 = (s) => `'${s.replace(/\r\n|[\r\n]/g, " ").replace(/'/g, `'"'"'`)}'`;
  const drill = (cmd, q) => `construct ${cmd} --out ${runDir} --q ${shellQuote2(q)}`;
  const bySource = {};
...
import { isAbsolute as isAbsolute2, join as join30, resolve as resolve5 } from "path";
...
function isTestFile2(rel2) {
  return TEST_FILE_RE.test(rel2) || TEST_SUFFIX_RE.test(rel2) || TEST_DIR_RE.test(rel2);
}
...
function verifyRun(runDir, opts = {}) {
  ...
  const appDir = rawApp ? isAbsolute2(rawApp) ? rawApp : resolve5(runDir, rawApp) : void 0;
  ...
  for (const t of doneTasks) {
    for (const rel2 of [...t.artifacts, ...t.tests]) {
      if (!existsSync16(join30(appDir, rel2))) errors.push(`${t.id} is done but its declared file is missing: ${rel2}.`);
    }
...
Worklist: \`${join31(runAbs, "VERIFY.todo.json")}\` (\`{ pairs: [...] }\`; each pair has \`claimId\`, \`kind\`, \`claim\`, \`evidenceId\`, \`source\`, \`digest\`). Handle ONLY the pairs whose \`claimId::evidenceId\` key is named in your prompt (\`PAIRS=<key,\u2026>\`). If a PAIRS key is no longer in the worklist, skip it and say so in your note.

For EACH of your pairs:

1. Read the pair's \`claim\` and its \`digest\` (the cited item's snippet). You may open the evidence source URL (see \`${join31(runAbs, "evidence", "EVIDENCE.md")}\`) for more context. A digest flagged \`[low-signal snippet \u2026]\` must be adjudicated skeptically \u2014 never grant \`supported\` on the URL alone.
...
You build ONE task of \`${join31(runAbs, "BUILD-PLAN.json")}\`, test-first, in your OWN isolated git worktree (references/orchestration.md Pattern 5 + references/build-playbook.md). Your prompt names your task (\`TASK=<id>\`). If your TASK id is no longer in the worklist, skip it and say so in your summary.

1. Read your task in the plan. Its \`acceptance\` entries POINT into \`${join31(runAbs, "SRD.json")}\` (\`functional[frId].acceptance[index]\`) \u2014 the SRD stays the single source of truth for what "done" means.
...
function runbookPreamble(phases, runAbs, engineAbs) {
  ...
  const agents = join31(runAbs, "orchestration", "agents");
  return [
...
1. **Interview \u2192 brief** (if not done): \`${engine} init --idea "<one-liner>" --out ${runAbs}\`, then fill \`${join31(runAbs, "brief.json")}\` one question at a time (references/interview-playbook.md).
2. **Research, then dig every gap** \u2014 \`${engine} research --out ${runAbs}\` builds the dossier; \`${engine} analyze --out ${runAbs}\` names each gap + its drill command. For EVERY gap, apply \`${join31(agents, "researcher.md")}\` yourself (run the drill, WebSearch what it misses, keep the URLs worth grounding). Fold in serially with ONE pinned re-run: \`${engine} research --out ${runAbs} --angles market,oss,tech --url <u,...>\` \u2192 re-run \`analyze\`. Loop until clean or the user stops you.
3. **Render**: \`${engine} render --out ${runAbs} --level complex\`, then enrich the SRD (SKILL.md step 4).
4. **Claim-support review** \u2014 \`${engine} review --out ${runAbs}\` writes \`${join31(runAbs, "VERIFY.todo.json")}\`. For EVERY pair, apply \`${join31(agents, "claim-reviewer.md")}\` yourself (verdict + note into a \`verdicts.json\`). Then fold: \`${engine} review --apply verdicts.json --out ${runAbs}\` and gate: \`${engine} check --out ${runAbs} --semantic\` (must exit 0 before presenting).
5. **Judge panel \u2014 only for ONE genuinely contested ADR** \u2014 apply \`${join31(agents, "adr-judge.md")}\` yourself three times (feasibility / operations-cost / user-value) over the pasted ADR + its cited evidence. Majority (\u22652 lenses \u22653) \u2192 one line per lens under *Alternatives considered*, flip \`proposed \u2192 accepted\` in \`${join31(runAbs, "SRD.json")}\`, re-emit: \`${engine} render --out ${runAbs} --from-srd\`.
6. **Build the frontier** \u2014 per ready task (\`${engine} status --out ${runAbs} --json\` \u2192 \`frontier\`), apply \`${join31(agents, "builder.md")}\` yourself (sequentially you may work in the app dir directly \u2014 no worktree needed); fold artifacts/tests/status into \`${join31(runAbs, "BUILD-PLAN.json")}\`, then \`${engine} verify --out ${runAbs}\`. Milestone gate once the frontier is folded: \`${engine} verify --out ${runAbs} --run-tests --strict\`.
...
With subagents available, prefer the emitted workflows instead: \`orchestrate --out ${runAbs} --phase <p>\` then \`Workflow({ scriptPath: "${join31(runAbs, "orchestration", "<p>.workflow.mjs")}" })\` \u2014 you stay the sole writer either way.
`
  ];
}
...
function researchUnits(runDir, engineAbs) {
  if (!existsSync17(join33(runDir, "brief.json")) || !existsSync17(join33(runDir, "evidence", "evidence.json"))) return null;
  try {
...
function loadSrd(runDir) {
  const manifest = srdManifestPath(runDir);
  if (!existsSync17(manifest)) return null;
  try {
    const srd = JSON.parse(readFileSync18(manifest, "utf8"));
...
function loadDossier(runDir) {
  const path = join33(runDir, "evidence", "evidence.json");
...
var RESEARCH = {
  name: "research",
  worklist: join33("evidence", "evidence.json"),
  ...
  ids: (_parsed, run2, engineAbs) => researchUnits(run2, engineAbs) ?? void 0,
  prerequisite: (run2, engineAbs) => `node ${engineAbs} research --out ${run2}`,
  role: "researcher",
  ...
  applyHint: (run2, engineAbs) => [
    `node ${engineAbs} research --out ${run2} --angles market,oss,tech --url <u1,u2,...> [--docs-url <d,...>]`,
    `node ${engineAbs} analyze --out ${run2}`
  ]
};
var CLAIM_REVIEW = {
  name: "claim-review",
  worklist: "VERIFY.todo.json",
  ids: (todo) => Array.isArray(todo?.pairs) ? todo.pairs.filter((p) => !!p && typeof p.claimId === "string" && typeof p.evidenceId === "string").map((p) => `${p.claimId}::${p.evidenceId}`) : void 0,
  prerequisite: (run2, engineAbs) => `node ${engineAbs} review --out ${run2}`,
  role: "claim-reviewer",
  title: "Claim review",
  ...
  applyHint: (run2, engineAbs) => [`node ${engineAbs} review --apply verdicts.json --out ${run2}`, `node ${engineAbs} check --out ${run2} --semantic`]
};
var ADR_JUDGES = {
  name: "adr-judges",
  worklist: join33("srd", "manifest.json"),
  ids: (_parsed, run2) => {
  ...
  prerequisite: (run2, engineAbs) => `node ${engineAbs} render --out ${run2} --level complex`,
  role: "adr-judge",
  title: "Judge panel",
  ...
  applyHint: (run2, engineAbs) => [`node ${engineAbs} render --out ${run2} --from-srd`]
};
var BUILD = {
  name: "build",
  ...
  ids: (_parsed, run2) => {
    ...
    return plan ? readyFrontier(plan).frontier : void 0;
  },
  prerequisite: (run2, engineAbs) => `node ${engineAbs} render --out ${run2} --level complex`,
  role: "builder",
  ...
  collapseFloor: () => 1,
  applyHint: (run2, engineAbs) => [`node ${engineAbs} verify --out ${run2}`]
};
var PHASE_DEFS = [RESEARCH, CLAIM_REVIEW, ADR_JUDGES, BUILD];
...
function emitOrchestration(runDir, engineAbs, opts = {}) {
  const run2 = resolve6(runDir);
  const phases = listPhasesFor(run2, engineAbs);
  const adrPhase = phases.find((p) => p.name === "adr-judges");
  const err2 = (errors) => ({ exitCode: 2, written: [], notices: [], errors, phases });
  let idea = "";
  try {
    idea = loadBrief(run2).idea;
  } catch {
  }
  const contracts = (r, e) => agentContracts(r, e, idea);
  if (opts.phase === void 0) {
    const notices = [];
    if (adrPhase?.ready) {
      notices.push(
    ...
    const res = orchestrateRun(
      run2,
      engineAbs,
      PHASE_DEFS.filter((d) => d.name !== "adr-judges"),
      contracts,
      { ...opts, runbookPreamble: runbookPreamble(phases, run2, engineAbs) }
    );
    return { ...res, notices: [...notices, ...res.notices], phases };
  }
  if (opts.phase === "adr-judges") {
    const ph = adrPhase;
    if (!ph?.ready) return err2([`phase "adr-judges" is not ready \u2014 render the SRD first: node ${engineAbs} render --out ${run2} --level complex`]);
    ...
    if (!ph.ids.includes(opts.adr)) return err2([`ADR "${opts.adr}" not found \u2014 ${available}.`]);
    const payload = adrPanelPayload(run2, opts.adr);
    if (!payload) return err2([`ADR "${opts.adr}" could not be loaded from ${ph.worklist}.`]);
    const panel = { ...ADR_JUDGES, ids: () => [...ADR_LENSES], collapseFloor: () => 1 };
    return orchestrateRun(run2, engineAbs, [panel], contracts, {
      ...opts,
      runbookPreamble: runbookPreamble(phases, run2, engineAbs),
      constants: { ADR: payload.adr, EVIDENCE: payload.evidence }
    });
  }
  return orchestrateRun(run2, engineAbs, PHASE_DEFS, contracts, { ...opts, runbookPreamble: runbookPreamble(phases, run2, engineAbs) });
}

// src/mcp/handlers.ts
import { existsSync as existsSync18, readFileSync as readFileSync19, realpathSync as realpathSync3, statSync as statSync7 } from "fs";
import { isAbsolute as isAbsolute3, join as join34, resolve as resolve7, sep as sep4 } from "path";
...
function requiredRun(args2, defaults) {
  ...
  const abs = resolve7(run2);
  if (!existsSync18(join34(abs, "brief.json"))) {
...
async function callTool2(name2, args2, defaults = {}) {
  ...
  if (name2 === "construct_cache") return outcome(name2, cacheStats());
...
function handleInit(args2) {
  ...
  const path = saveBrief(resolve7(out2), brief);
  return {
    run: resolve7(out2),
    brief: path,
...
function handleStatus(run2) {
  const has = (rel2) => existsSync18(join34(run2, rel2));
  return {
    run: run2,
...
function handleVerify(args2, run2) {
  ...
  if (app && !existsSync18(app)) throw new ToolError(`app not found: ${app}`);
...
function constructAdapter(opts = {}) {
  return {
    version: VERSION,
...
function buildResearchContext(p, runDir, angles) {
  ...
  setCacheMode({ refresh: p.bools.has("refresh"), offline: p.bools.has("offline") });
...
async function main() {
  ...
  switch (p.command) {
    case "init": {
      ...
      const out2 = p.values.out ? resolve8(p.values.out) : resolve8(slugify(idea) || "construct-run");
      const brief = initBrief(idea, (/* @__PURE__ */ new Date()).toISOString());
    ...
    case "brainstorm": {
      ...
      process.stderr.write(
        [
          `construct: brainstorm board at ${join35(out2, "BRAINSTORM.md")}`,
...
`);
    ...
    case "render": {
      ...
      if (p.bools.has("from-srd")) {
        ...
        process.stderr.write(
          [
            `construct: re-emitted the SRD tree from ${join35(out2, "SRD.json")}`,
...
${v.errors.map((e) => "  - " + e).join("\n")}`);
    ...
    case "review": {
      const out2 = requireOut(p);
      if (p.values.apply) {
        const res = applyVerdicts(out2, resolve8(p.values.apply));
...
`
    ...
    case "verify": {
      ...
      const res = verifyRun(out2, {
        appDir: p.values.app ? resolve8(p.values.app) : void 0,
    ...
    case "status": {
      ...
      const has = (rel2) => existsSync19(join35(out2, rel2)) ? "\u2713" : "\xB7";
...
`);
        ...
        process.stderr.write(`Follow ${join35(runDir, "orchestration", "RUNBOOK.md")} sequentially (the eco path).
`);
      }
      return;
    }
...
`);
    ...
    case "cache": {
      ...
      if (action === "clean") {
        ...
        const out2 = { action: "clean", all, removed, dir: cacheStats().dir };
...
`);
      ...
      process.stdout.write(
        [
          ...
          `  ttl:      ${st.ttlMs / 36e5} h (CONSTRUCT_CACHE_TTL_HOURS)`,
...
function nextCommand(out2, plan) {
  const has = (rel2) => existsSync19(join35(out2, rel2));
