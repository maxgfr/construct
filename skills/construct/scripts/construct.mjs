...
function encode(model, text) {
  ...
  for (let d = 0; d < dim; d++) {
    let q = roundHalfToEven(pooled[d] / norm22 * QUANT);
    if (q > QUANT) q = QUANT;
...
function searchSemantic(scan2, query2, index, opts = {}) {
  ...
  const q = encode(opts.model, query2);
  ...
  if (name2 === "search") {
    ...
    if (args2.semantic === true) {
      ...
      if (model) {
        ...
        return JSON.stringify(searchSemantic(scan2, query2, index, { model, limit, fuzzy }), null, 2);
      }
    }
