...
// src/cli.ts
import { resolve as resolve5, join as join29 } from "path";
...
function collectCalls(root, spec, maxCalls = MAX_CALLS) {
  ...
  const add = (name2, node, receiver) => {
    ...
    seen.add(key);
    out2.push(receiver ? { name: name2, line: line2, receiver } : { name: name2, line: line2 });
  };
  const visit = (node) => {
...
function readTsConfig(root, fileSet, rel, warnings, seen) {
  ...
  seen.add(rel);
  const cfg = tolerantJsonParse(readText(join42(root, rel)));
  if (cfg === void 0) {
...
function buildResolveContext(scan2) {
  ...
  for (const rel of fileSet) {
    ...
    const text = readText(join42(scan2.root, rel));
  ...
  for (const rel of fileSet) {
    ...
    const pkg = tolerantJsonParse(readText(join42(scan2.root, rel)));
    if (pkg === void 0) {
  ...
  for (const rel of fileSet) {
    ...
    const composer = tolerantJsonParse(readText(join42(scan2.root, rel)));
    if (!composer) {
...
function probeNodePkg(root, dir, kind, warnings) {
  ...
  if (!existsSync32(path)) return void 0;
...
function probeGradle(root, dir) {
  for (const f of ["build.gradle", "build.gradle.kts"]) {
    if (existsSync32(join11(root, dir, f))) {
...
function resolveEmbedModelDir(repo) {
  ...
  for (const c2 of candidates) {
    if (existsSync42(join13(c2, "model.json"))) return c2;
  }
  return void 0;
}
...
`);
  ...
  if (existsSync22(runtime) && expected && existsSync22(markerPath)) {
    let marker = "";
    try {
      marker = readFileSync32(markerPath, "utf8").trim();
    } catch {
    }
    if (marker === expected) {
...
function composeFile() {
  ...
  for (const cand of [join16(here, "..", "docker-compose.yml"), join16(here, "docker-compose.yml"), join16(here, "..", "..", "docker-compose.yml")]) {
...
function loadLedger(dir) {
  const p = join17(dir, "ids.json");
  if (!existsSync7(p)) return emptyLedger();
  try {
...
function writeDossier(dir, evidence, meta, ledger = emptyLedger()) {
  ...
  const idsJson = join17(dir, "ids.json");
...
async function runResearch(ctx, builtAt) {
  ...
  const dir = join18(ctx.runDir, "evidence");
...
function buildPlanPath(runDir) {
  return join20(runDir, "BUILD-PLAN.json");
}
...
function renderFromSRD(runDir, opts) {
  ...
  let srd;
  try {
    srd = JSON.parse(readFileSync11(manifest, "utf8"));
  } catch (e) {
...
function emitSRD(srd, opts) {
  const files = [];
  const out2 = opts.out;
  if (!opts.prd && !opts.noPrd && existsSync9(join21(out2, "requirements", "prd"))) {
  ...
  rmSync4(join21(out2, "architecture", "decisions"), { recursive: true, force: true });
  rmSync4(join21(out2, "design"), { recursive: true, force: true });
  rmSync4(join21(out2, "prd"), { recursive: true, force: true });
  ...
  rmSync4(join21(out2, "requirements", "prd"), { recursive: true, force: true });
  if (opts.prd) {
    for (const fr of srd.functional) {
  ...
  if (opts.merge) {
    ...
    rmSync4(join21(out2, "SRD.md"), { force: true });
  }
  return { dir: out2, files, srd };
}

// src/check.ts
import { existsSync as existsSync11, readFileSync as readFileSync13, readdirSync as readdirSync4, statSync as statSync5 } from "fs";
import { join as join24, relative, sep as sep3 } from "path";
...
function runReview(runDir, opts = {}) {
  const manifest = srdManifestPath(runDir);
  if (!existsSync10(manifest)) throw new Error(`No SRD.json in ${runDir} \u2014 render the SRD first (construct render).`);
  let srd;
  try {
  ...
  const evidence = loadEvidence(join23(runDir, "evidence", "evidence.json"));
  ...
  writeFileSync8(join23(runDir, "VERIFY.todo.json"), JSON.stringify(todo, null, 2));
  writeFileSync8(join23(runDir, "VERIFY.md"), renderWorklistMd(worklist, pairs.length, dropped));
  return worklist;
}
...
function applyVerdicts(runDir, verdictsPath) {
  if (!existsSync10(verdictsPath)) throw new Error(`verdicts file not found: ${verdictsPath}`);
  let raw;
  try {
  ...
  const todoPath = join23(runDir, "VERIFY.todo.json");
  if (existsSync10(todoPath)) {
    try {
  ...
  writeFileSync8(join23(runDir, "VERIFY.json"), JSON.stringify({ ...result, ...srdGeneratedAt ? { srdGeneratedAt } : {}, verdicts }, null, 2));
...
function readSrdGeneratedAt(runDir) {
  const p = join23(runDir, "SRD.json");
  if (!existsSync10(p)) return void 0;
  try {
    const srd = JSON.parse(readFileSync12(p, "utf8"));
...
function countProposedIdeas(runDir) {
  const p = join24(runDir, "brainstorm.json");
  if (!existsSync11(p)) return 0;
  try {
    const data = JSON.parse(readFileSync13(p, "utf8"));
...
function loadEvidence2(runDir) {
  const path = join24(runDir, "evidence", "evidence.json");
...
function uncoveredPairs(runDir, verdicts) {
  ...
  const todoPath = join24(runDir, "VERIFY.todo.json");
...
function applySemantic(runDir, result, allowUnverified) {
  const p = join24(runDir, "VERIFY.json");
...
function checkDesign(runDir, srd, errors, warnings) {
  ...
  for (const f of DESIGN_REQUIRED_FILES) {
    if (!existsSync11(join24(runDir, f))) errors.push(`Missing required design file: ${f} (re-render at --level complex).`);
  }
  ...
  const tokenDoc = join24(runDir, "design", "DESIGN-TOKENS.md");
  if (existsSync11(tokenDoc) && readFileSync13(tokenDoc, "utf8").includes(DESIGN_TOKENS_SEEDED_BANNER)) {
...
function checkModules(runDir, srd, errors, warnings) {
  ...
  if (!existsSync11(join24(runDir, "prd", "README.md"))) {
    errors.push(`Missing required module-PRD index: prd/README.md (re-render).`);
  }
  for (const m of mods) {
    if (!existsSync11(join24(runDir, "prd", m.id, "PRD.md"))) {
...
function checkRun(runDir, opts = {}) {
  ...
  for (const f of REQUIRED_FILES) {
    if (!existsSync11(join24(runDir, f))) errors.push(`Missing required file: ${f} (run \`construct render --out ${runDir}\`).`);
  }
  const manifest = srdManifestPath(runDir);
  ...
  for (const rel of mdFiles(runDir)) {
    const text = readFileSync13(join24(runDir, rel), "utf8");
  ...
  if (opts.semantic) {
    ...
    result.semanticSkipped = { citedClaims, verifyExists: existsSync11(join24(runDir, "VERIFY.json")) };
  }
  return result;
}
...
function loadEvidence3(runDir) {
  const path = join25(runDir, "evidence", "evidence.json");
...
function loadMetaNotes(runDir) {
  const path = join25(runDir, "evidence", "meta.json");
  if (!existsSync12(path)) return [];
  try {
    const meta = JSON.parse(readFileSync14(path, "utf8"));
...
import { isAbsolute, join as join26, resolve as resolve3 } from "path";
...
function verifyRun(runDir, opts = {}) {
  ...
  for (const t of doneTasks) {
    for (const rel of [...t.artifacts, ...t.tests]) {
      if (!existsSync13(join26(appDir, rel))) errors.push(`${t.id} is done but its declared file is missing: ${rel}.`);
    }
...
Exception for oversized prose: if a justification is too large to return, write ONLY to \`${join27(runAbs, "orchestration", "out")}/<role>-<batch>.md\` (a file namespaced to you alone) and return its path.
`;
}
var RESEARCH_SCHEMA = {
  type: "object",
...
Worklist: \`${join27(runAbs, "VERIFY.todo.json")}\` (\`{ pairs: [...] }\`; each pair has \`claimId\`, \`kind\`, \`claim\`, \`evidenceId\`, \`source\`, \`digest\`). Handle ONLY the pairs whose \`claimId::evidenceId\` key is named in your prompt (\`PAIRS=<key,\u2026>\`). If a PAIRS key is no longer in the worklist, skip it and say so in your note.

For EACH of your pairs:

1. Read the pair's \`claim\` and its \`digest\` (the cited item's snippet). You may open the evidence source URL (see \`${join27(runAbs, "evidence", "EVIDENCE.md")}\`) for more context. A digest flagged \`[low-signal snippet \u2026]\` must be adjudicated skeptically \u2014 never grant \`supported\` on the URL alone.
...
You build ONE task of \`${join27(runAbs, "BUILD-PLAN.json")}\`, test-first, in your OWN isolated git worktree (references/orchestration.md Pattern 5 + references/build-playbook.md). Your prompt names your task (\`TASK=<id>\`). If your TASK id is no longer in the worklist, skip it and say so in your summary.

1. Read your task in the plan. Its \`acceptance\` entries POINT into \`${join27(runAbs, "SRD.json")}\` (\`functional[frId].acceptance[index]\`) \u2014 the SRD stays the single source of truth for what "done" means.
...
function runbookMd(phases, runAbs, engineAbs) {
  ...
  const agents = join27(runAbs, "orchestration", "agents");
...
1. **Interview \u2192 brief** (if not done): \`${engine} init --idea "<one-liner>" --out ${runAbs}\`, then fill \`${join27(runAbs, "brief.json")}\` one question at a time (references/interview-playbook.md).
2. **Research, then dig every gap** \u2014 \`${engine} research --out ${runAbs}\` builds the dossier; \`${engine} analyze --out ${runAbs}\` names each gap + its drill command. For EVERY gap, apply \`${join27(agents, "researcher.md")}\` yourself (run the drill, WebSearch what it misses, keep the URLs worth grounding). Fold in serially with ONE pinned re-run: \`${engine} research --out ${runAbs} --angles market,oss,tech --url <u,...>\` \u2192 re-run \`analyze\`. Loop until clean or the user stops you.
3. **Render**: \`${engine} render --out ${runAbs} --level complex\`, then enrich the SRD (SKILL.md step 4).
4. **Claim-support review** \u2014 \`${engine} review --out ${runAbs}\` writes \`${join27(runAbs, "VERIFY.todo.json")}\`. For EVERY pair, apply \`${join27(agents, "claim-reviewer.md")}\` yourself (verdict + note into a \`verdicts.json\`). Then fold: \`${engine} review --apply verdicts.json --out ${runAbs}\` and gate: \`${engine} check --out ${runAbs} --semantic\` (must exit 0 before presenting).
5. **Judge panel \u2014 only for ONE genuinely contested ADR** \u2014 apply \`${join27(agents, "adr-judge.md")}\` yourself three times (feasibility / operations-cost / user-value) over the pasted ADR + its cited evidence. Majority (\u22652 lenses \u22653) \u2192 one line per lens under *Alternatives considered*, flip \`proposed \u2192 accepted\` in \`${join27(runAbs, "SRD.json")}\`, re-emit: \`${engine} render --out ${runAbs} --from-srd\`.
6. **Build the frontier** \u2014 per ready task (\`${engine} status --out ${runAbs} --json\` \u2192 \`frontier\`), apply \`${join27(agents, "builder.md")}\` yourself (sequentially you may work in the app dir directly \u2014 no worktree needed); fold artifacts/tests/status into \`${join27(runAbs, "BUILD-PLAN.json")}\`, then \`${engine} verify --out ${runAbs}\`. Milestone gate once the frontier is folded: \`${engine} verify --out ${runAbs} --run-tests --strict\`.
...
With subagents available, prefer the emitted workflows instead: \`orchestrate --out ${runAbs} --phase <p>\` then \`Workflow({ scriptPath: "${join27(runAbs, "orchestration", "<p>.workflow.mjs")}" })\` \u2014 you stay the sole writer either way.
`;
}
...
function loadSrd(runDir) {
  const manifest = srdManifestPath(runDir);
  if (!existsSync14(manifest)) return null;
  try {
    const srd = JSON.parse(readFileSync16(manifest, "utf8"));
...
function researchUnits(runDir, engineAbs) {
  if (!existsSync14(join28(runDir, "brief.json")) || !existsSync14(join28(runDir, "evidence", "evidence.json"))) return null;
  try {
...
function listPhases(runDir, engineAbs) {
  ...
  const todoPath = join28(run2, "VERIFY.todo.json");
  let pairKeys = null;
  ...
  return [
    {
      name: "research",
      ready: gaps !== null,
      worklist: join28(run2, "evidence", "evidence.json"),
      items: gaps?.length ?? 0,
      ids: gaps ?? [],
    ...
    },
    {
      name: "build",
      ready: frontier !== null,
      worklist: join28(run2, "BUILD-PLAN.json"),
...
async function main() {
  ...
  switch (p.command) {
    ...
    case "brainstorm": {
      ...
      process.stderr.write(
        [
          `construct: brainstorm board at ${join29(out2, "BRAINSTORM.md")}`,
...
`);
    ...
    case "render": {
      ...
      if (p.bools.has("from-srd")) {
        ...
        process.stderr.write(
          [
            `construct: re-emitted the SRD tree from ${join29(out2, "SRD.json")}`,
...
`
    ...
    case "status": {
      ...
      const has = (rel) => existsSync15(join29(out2, rel)) ? "\u2713" : "\xB7";
...
`);
        ...
        process.stderr.write(`Follow ${join29(runDir, "orchestration", "RUNBOOK.md")} sequentially (the eco path).
`);
      }
      return;
    }
