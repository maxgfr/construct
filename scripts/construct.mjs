...
import { readFileSync as readFileSync5, writeFileSync as writeFileSync22 } from "fs";
import { join as join11 } from "path";
import { mkdirSync as mkdirSync22, readdirSync as readdirSync22, readFileSync as readFileSync6, rmSync as rmSync22, statSync as statSync32, writeFileSync as writeFileSync32 } from "fs";
import { dirname as dirname4, join as join12 } from "path";
import { existsSync as existsSync42, readdirSync as readdirSync32, statSync as statSync4 } from "fs";
import { join as join13 } from "path";
import { createHash as createHash3 } from "crypto";
import { existsSync as existsSync52, readFileSync as readFileSync7 } from "fs";
...
import { existsSync as existsSync32, mkdirSync as mkdirSync5, mkdtempSync, readFileSync as readFileSync42, renameSync, rmSync as rmSync3, writeFileSync as writeFileSync4 } from "fs";
import { dirname as dirname3, join as join52, resolve as resolve2, sep as sep2 } from "path";
    ...
    const walk22 = (node, parent, exported) => {
      ...
      if (spec.assignments && node.type === "assignment_statement") {
        ...
        for (let i2 = 0; i2 < pairs; i2++) {
          ...
          const line2 = firstLine(node, content);
          symbols.push({
            name: target.text,
            kind: "function",
            file: rel,
            ...
            ...parent ? { parent } : {},
            signature: line2,
            exported: nowExported || spec.exported(line2, target.text),
            lang: spec.lang
          });
        }
        return;
...
function probeNodePkg(root, dir, kind, warnings) {
  ...
  if (!existsSync42(path)) return void 0;
...
function probeGradle(root, dir) {
  for (const f of ["build.gradle", "build.gradle.kts"]) {
    if (existsSync42(join13(root, dir, f))) {
...
function subdirsOf(root, base) {
  let entries;
  try {
    entries = readdirSync32(base ? join13(root, base) : root, { withFileTypes: true });
  } catch {
    return [];
  }
...
function resolveEmbedModelDir(repo) {
  ...
  for (const c2 of candidates) {
    if (existsSync52(join15(c2, "model.json"))) return c2;
  }
  return void 0;
}
...
async function callTool(name2, args2, defaultRepo) {
  ...
  if (name2 === "search") {
    ...
    if (args2.semantic === true) {
      ...
      if (endpoint) {
        try {
          ...
          const results2 = searchIndex(scan2, query2, { limit, fuzzy });
          return JSON.stringify(
      ...
      const results = searchIndex(scan2, query2, { limit, fuzzy });
      return JSON.stringify(
    ...
    return JSON.stringify(searchIndex(scan2, query2, { limit, fuzzy }), null, 2);
  }
...
`);
  ...
  const runtime = join52(cacheDir2, "web-tree-sitter.wasm");
  const markerPath = join52(dirname3(cacheDir2), `${ENGINE_VERSION}.sha256`);
  if (existsSync32(runtime) && expected && existsSync32(markerPath)) {
    let marker = "";
    try {
      marker = readFileSync42(markerPath, "utf8").trim();
    } catch {
    }
    if (marker === expected) {
...
`
  ...
  try {
    ...
    tmp = mkdtempSync(join52(dirname3(cacheDir2), ".grammars-tmp-"));
    extractGrammarsTarball(bytes, tmp);
    if (!existsSync32(join52(tmp, "web-tree-sitter.wasm"))) {
      throw new Error("archive is missing web-tree-sitter.wasm");
    }
    if (existsSync32(cacheDir2)) rmSync3(cacheDir2, { recursive: true, force: true });
...
function composeFile() {
  ...
  for (const cand of [join20(here, "..", "docker-compose.yml"), join20(here, "docker-compose.yml"), join20(here, "..", "..", "docker-compose.yml")]) {
...
import { existsSync as existsSync9, mkdirSync as mkdirSync6, readFileSync as readFileSync10, writeFileSync as writeFileSync5 } from "fs";
...
function writeDossier(dir, evidence, meta, ledger = emptyLedger()) {
  ...
  writeFileSync5(idsJson, JSON.stringify(ledger, null, 2));
...
// src/render.ts
import { existsSync as existsSync11, mkdirSync as mkdirSync7, readFileSync as readFileSync12, writeFileSync as writeFileSync7, rmSync as rmSync4 } from "fs";
...
function renderFromSRD(runDir, opts) {
  ...
  let srd;
  try {
    srd = JSON.parse(readFileSync12(manifest, "utf8"));
  } catch (e) {
...
function emitSRD(srd, opts) {
  ...
  rmSync4(join26(out2, "architecture", "decisions"), { recursive: true, force: true });
  rmSync4(join26(out2, "design"), { recursive: true, force: true });
  rmSync4(join26(out2, "prd"), { recursive: true, force: true });
  ...
  rmSync4(join26(out2, "requirements", "prd"), { recursive: true, force: true });
  if (opts.prd) {
    for (const fr of srd.functional) {
  ...
  writeFileSync7(srdManifestPath(out2), JSON.stringify(srd, null, 2) + "\n");
  files.push("SRD.json");
  if (opts.merge) {
    writeFile(out2, "SRD.md", renderMergeBundle(srd), files);
  } else {
    rmSync4(join26(out2, "SRD.md"), { force: true });
  }
  return { dir: out2, files, srd };
}

// src/check.ts
import { existsSync as existsSync13, readFileSync as readFileSync14, readdirSync as readdirSync4, statSync as statSync6 } from "fs";
import { join as join28, relative, sep as sep3 } from "path";

// src/review.ts
import { existsSync as existsSync12, readFileSync as readFileSync13, writeFileSync as writeFileSync8 } from "fs";
...
function runReview(runDir, opts = {}) {
  ...
  writeFileSync8(join27(runDir, "VERIFY.todo.json"), JSON.stringify(todo, null, 2));
  writeFileSync8(join27(runDir, "VERIFY.md"), renderWorklistMd(worklist, pairs.length, dropped));
  return worklist;
}
...
function applyVerdicts(runDir, verdictsPath) {
  ...
  let raw;
  try {
    raw = JSON.parse(readFileSync13(verdictsPath, "utf8"));
  } catch (e) {
  ...
  writeFileSync8(join27(runDir, "VERIFY.json"), JSON.stringify({ ...result, ...srdGeneratedAt ? { srdGeneratedAt } : {}, verdicts }, null, 2));
...
function readSrdGeneratedAt(runDir) {
  ...
  try {
    const srd = JSON.parse(readFileSync13(p, "utf8"));
...
function mdFiles(runDir) {
  ...
  while (stack.length) {
    ...
    for (const name2 of entries) {
      ...
      const rel = relative(runDir, abs).split(sep3).join("/");
      if (st.isDirectory()) {
        if (rel === "evidence" || rel === "orchestration" || name2 === ".construct") continue;
        stack.push(abs);
...
function countProposedIdeas(runDir) {
  ...
  try {
    const data = JSON.parse(readFileSync14(p, "utf8"));
...
function checkDesign(runDir, srd, errors, warnings) {
  ...
  if (existsSync13(tokenDoc) && readFileSync14(tokenDoc, "utf8").includes(DESIGN_TOKENS_SEEDED_BANNER)) {
...
function checkRun(runDir, opts = {}) {
  ...
  for (const rel of mdFiles(runDir)) {
    const text = readFileSync14(join28(runDir, rel), "utf8");
...
function loadMetaNotes(runDir) {
  ...
  if (!existsSync14(path)) return [];
  try {
    const meta = JSON.parse(readFileSync15(path, "utf8"));
...
import { isAbsolute as isAbsolute2, join as join30, resolve as resolve3 } from "path";
...
function verifyRun(runDir, opts = {}) {
  ...
  for (const t of doneTasks) {
    for (const rel of [...t.artifacts, ...t.tests]) {
      if (!existsSync15(join30(appDir, rel))) errors.push(`${t.id} is done but its declared file is missing: ${rel}.`);
    }
...
function orchestrateRun(runDir, engineAbs, opts = {}) {
  ...
  mkdirSync8(join33(orchDir, "out"), { recursive: true });
  ...
  if (!opts.eco) {
    for (const ph of selected) {
      ...
      writeFileSync9(p, phaseWorkflowScript(ph, run2, engineAbs, units, adrPayload));
      written.push(p);
    }
  }
  const rb = join33(orchDir, "RUNBOOK.md");
  writeFileSync9(rb, runbookMd(phases, run2, engineAbs));
  written.push(rb);
