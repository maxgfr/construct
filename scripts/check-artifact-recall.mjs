...
const TRACKED = [
  // The example SRD: its requirements, build plan and traceability are rendered
  // from evidence the vendored engines gathered, so an engine bump legitimately
  // moves it. `pnpm run example` also re-checks grounding >= 66, which catches a
  // collapse this guard would not.
  "assets/example-srd",
];
