...
if (!existsSync(skillMd)) {
  ...
  if (!fm) bad("skills/" + name + "/SKILL.md has no frontmatter block");
    ...
    if (!desc) bad("frontmatter has no description");
      ...
      len <= DESC_MAX ? ok(`description ${len} chars (<= ${DESC_MAX} headroom cap)`) : bad(`description ${len} chars exceeds the ${DESC_MAX}-char headroom cap (matcher hard-limit is 1024)`);
    }
  }
