...
// process — the exact file `claude mcp add -- node scripts/construct.mjs mcp`
...
const BUNDLE = resolve("scripts/construct.mjs");
const temps: string[] = [];
...
describe("the bundled MCP server over stdio", () => {
  it("completes a handshake, and writes NOTHING to stdout but JSON-RPC frames", async () => {
    ...
    expect(msgs[0].result.serverInfo.name).toBe("construct");
    ...
    expect(names).toContain("construct_render");
    expect(names).toContain("construct_check");
    // The write tool stays hidden without --allow-write.
    expect(names).not.toContain("construct_init");
    expect(s.code).toBe(0);
  });

  it("runs a real tool call, offline", async () => {
    const s = await session([INIT, { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "construct_cache", arguments: {} } }]);
    const call = s.lines.map((l) => JSON.parse(l)).find((m) => m.id === 2);
    expect(JSON.parse(call.result.content[0].text).dir).toBeTruthy();
  });

  it("scaffolds a run through the bundle and reads the brief back", async () => {
    // The full loop an MCP client actually runs: init writes, read opens what
    // it wrote.
    const base = mkdtempSync(join(tmpdir(), "con-stdio-"));
    temps.push(base);
    ...
    const s = await session(
      [
        INIT,
        { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "construct_init", arguments: { idea: "a photo renamer", out: run } } },
        { jsonrpc: "2.0", id: 3, method: "tools/call", params: { name: "construct_read", arguments: { run, path: "brief.json" } } },
      ],
      { args: ["--allow-write"] },
    );
    ...
    expect(JSON.parse(msgs.find((m) => m.id === 3).result.content[0].text).content).toContain("photo renamer");
  });
  ...
  it("advertises and serves all three primitives from the committed bundle", async () => {
    ...
    const s = await session([
      ...
      { jsonrpc: "2.0", id: 5, method: "prompts/get", params: { name: "enrich_srd", arguments: { run: "/srv/run" } } },
    ]);
    const msgs = s.lines.map((l) => JSON.parse(l));
    ...
    expect(uris).toContain("skill://references/interview-playbook.md");

    expect(msgs.find((m) => m.id === 3).result.prompts.map((p: { name: string }) => p.name)).toEqual(["interview_idea", "enrich_srd", "judge_adr"]);

    const contents = msgs.find((m) => m.id === 4).result.contents[0];
    expect(contents.mimeType).toBe("text/markdown");
    expect(contents.text).toContain("construct");

    const rendered = msgs.find((m) => m.id === 5).result.messages[0].content.text;
    expect(rendered).toContain("/srv/run");
    expect(rendered).toContain("construct_check");
  ...
  it("reports a bad resource uri and a bad prompt name as invalid params", async () => {
    const s = await session([
      ...
      { jsonrpc: "2.0", id: 4, method: "prompts/get", params: { name: "enrich_srd", arguments: {} } },
...
describe("server flags, through the bundle", () => {
  it("makes `run` optional on every dossier tool when a default is configured", async () => {
    const s = await session([INIT, { jsonrpc: "2.0", id: 2, method: "tools/list" }], { args: ["--out", "/tmp/some-run"] });
