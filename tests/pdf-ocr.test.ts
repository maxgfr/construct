...
import { ocrPdf, ocrTools, ocrBudgetLeft, resetOcrBudget } from "../src/research/pdf/ocr.js";
...
beforeEach(() => vi.stubEnv("CONSTRUCT_OCR_MAX", "3"));
...
describe("ocrPdf", () => {
  ...
  it("honours CONSTRUCT_OCR_LANG", async () => {
    vi.stubEnv("CONSTRUCT_OCR_LANG", "fra+eng");
    toolsPresent();
    await ocrPdf(PDF);
...
describe("the per-process OCR budget", () => {
  ...
  it("stops after CONSTRUCT_OCR_MAX documents", async () => {
    vi.stubEnv("CONSTRUCT_OCR_MAX", "2");
    toolsPresent();
  ...
  it("can be switched off entirely with 0", async () => {
    vi.stubEnv("CONSTRUCT_OCR_MAX", "0");
    toolsPresent();
    expect(await ocrPdf(PDF)).toBeUndefined();
