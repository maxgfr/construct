...
import { extractDocument, enabledDocExtractors, resetDocLadderCache } from "../src/research/doc/ladder.js";
import { runWithInput, ANYDOC_SPEC } from "../src/research/pdf/exec.js";
...
// registry.
vi.mock("../src/research/pdf/exec.js", () => ({
...
describe("enabledDocExtractors", () => {
  it("defaults to the full ladder, strongest first", () => {
    vi.stubEnv("CONSTRUCT_DOC_ENGINE", undefined);
    expect(enabledDocExtractors()).toEqual(["anydoc", "firecrawl"]);
  });

  it("honours CONSTRUCT_DOC_ENGINE by running exactly that rung", () => {
    vi.stubEnv("CONSTRUCT_DOC_ENGINE", "firecrawl");
  ...
  it("empties the ladder on CONSTRUCT_DOC_ENGINE=none", () => {
    vi.stubEnv("CONSTRUCT_DOC_ENGINE", "none");
    expect(enabledDocExtractors()).toEqual([]);
  });

  it("drops the rung that needs an implicit install under CONSTRUCT_NO_NPX", () => {
    vi.stubEnv("CONSTRUCT_DOC_ENGINE", undefined);
    vi.stubEnv("CONSTRUCT_NO_NPX", "1");
    expect(enabledDocExtractors()).toEqual(["firecrawl"]);
  });

  it("ignores an unknown engine name rather than emptying the ladder", () => {
    vi.stubEnv("CONSTRUCT_DOC_ENGINE", "nope");
    expect(enabledDocExtractors()).toEqual(["anydoc", "firecrawl"]);
  });
